...
def test_poisson(j=np.array([1000, 100, 10]), l=3, t=100, lam=1, n_samples=int(1e3), seed=0):
    ...
    y = np.exp(y1 / 10)
    n = np.ones_like(y1)
    sampler = nfx.glm.poisson.sample_posterior(y, n, x, i, ome=ome)
    samples = [next(sampler) for _ in range(n_samples)]


def test_gaussian(j=np.array([1000, 100, 10]), l=3, t=100, lam=1, n_samples=int(1e3), seed=0):
